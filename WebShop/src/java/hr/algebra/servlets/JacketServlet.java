...
public class JacketServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet JacketServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet JacketServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        List<Item> jackets = repo.getItemsByCategory(3);
        HttpSession session = request.getSession();
        List<Category> categories = repo.getCategories();
        session.setAttribute("categories", categories);
        session.setAttribute("Jackets", jackets);
        response.sendRedirect("jacket.jsp");
    }
